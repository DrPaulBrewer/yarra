...
//    function ignore(){}
     ...
     * @return {Object} an object with fields dir and studies.  Studies is an array of strings that are file paths of config.json files.
     */
	...
	return (ls(buckets.sim, path)
		.then(studies)
		.then((obj)=>{
		    return pEachSeries(obj.studies, ((pathToStudyJSON)=>(func(pathToStudyJSON, obj.dir))));
		})
	       );
    }
    ...
    function missing(lsdir, pathToStudyJSON, numberOfConfigurations, fname){
	const n = +numberOfConfigurations;
	if (!(n>0)) throw new Error("numberOfConfigurations must be greater than zero, got: "+n);
	const paths = Study.paths(pathToStudyJSON, numberOfConfigurations, fname);
	return paths.filter((p)=>(!(lsdir.hasOwnProperty(p))));
    }

/**
 * returns a Promise resolving to an object containing information on each study in progress
 * @param {string} path
 * @return {Promise<object[]>} array of objects
 */
    
    function inProgress(path){
	const status = [];
	const zipdir = {};
	return (withEachStudy(path, (pathToStudyJSON, lsdir)=>{
	    const numberOfConfigurations = lsdir[pathToStudyJSON].metadata.metadata.numberOfConfigurations;
	    status.push(
		Object.assign(
		    {},
